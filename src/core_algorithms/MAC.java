...
public abstract class MAC<X, V> {
     ...
     *         * - If `tail`'s domain is empty, return false (inconsistency
     *         detected).
     *         * - Otherwise, add new arcs `(X, tail)` for each neighbor `X` of
     *         `tail`
     ...
     *         * - Use an additional data structure to track which arcs are
     *         currently in
     ...
     * variable
     * to consider next. The MRV heuristic selects the variable with the fewest
     * legal values
     * remaining in its domain, helping reduce the branching factor early in the
     * search.
     *
     ...
     *         - Among the unassigned variables, identifying the one with the
     *         smallest domain size.
     *         - Returning that variable.
     *
     *         Important:
     *         - If multiple variables have the same smallest domain size, you may
    ...
    // the two abstract methods below should be implemented in
    // BacktrackingSearch_Sudoku.java
